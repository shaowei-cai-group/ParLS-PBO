...
public:
	/***************PLS added****************/
	int adjust_rounds = 566024;
	double coeff = 1.0, coeff_inc = 1.0, coeff_dec = 1.0;
	int feasibles = 0;

	const double *global_score_0;
	const double *global_score_1;
	int gs_prob;
	const int *new_old_var;
	int id;
	int init_sharing;
	void *issuer;
	int terminated;
	int best_flips;
	int cccheck;   // whether use cc check
	int weighting; // whether use sophisticated weighting schema
	long long wecnf_obj_pre = 0;
	long long real_obj_pre = 0;
	unsigned int seed;
	long long opt_result;
	long long sharing_period;
	long long restart_period;
	long long punish_period;
	long long sharing_i;
	long long punish_i;
	long long restart_i;
	void (*cbkUpdateSolution)(void *, int, int *, int, long long);
	int (*cbkGetOtherModel)(void *, int, int, int *);
	void (*cbkPunish)(void *);
	int (*cbkUpdateRecent)(void *, int, int *, int);
	...
	void sat(int clause);
	void init(vector<int> &init_solution, int sharing = 0);
...
public:
	Satlike();
	void build_instance();
	int local_search_with_decimation(vector<int> &init_solution);
	void simple_print();
